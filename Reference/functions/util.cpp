...
SNode *variableAccess(SNode *variableID, SNode *indexExpr, SNode *Scope[2], int usedVariableType) {
	...
	for (int i = 0; i < 2; i++) {
		if (!Scope[i])
			continue;
		symbolNode *Table = Scope[i]->getTable(), *temp = nullptr;
		if (Table)
			temp = Table->lookUp(variableID->getName());
		if (temp == nullptr) {
			deb;
//			val(i);
//			val(Scope[i]);
//			val(Scope[i]->getEntry());
//			val(variableID->to_string());
			if(Scope[i]->getEntry()){
...
//					val(node->getName());
		...
		if (temp == nullptr)
			continue;
		int declaredVariableType = getUsageType(temp);
		...
		switch (usedVariableType) {
			case ARRAY: {
					variableID->setEntry(temp);
					variableID->setPTR1(indexExpr);
					variableID->setReturnType(temp->getType());
					return variableID;
					break;
				}
			case VARIABLE: {
					...
					if (arglist.size() != temp->getArgs().size()) {
						error("Arguments Mismatch at " + std::to_string(std::min(arglist.size(), temp->getArgs().size())) + "th argument", variableID->lineNo, variableID->colNo);
						return nullptr;
					}
...
int generateFunctionHead(SNode *func, FILE *output){
	...
	fprintf(output, "PUSH BP //After Function Call Begin\n");
	...
	fprintf(output, "MOV SP, R%d //After Function Call End\n", reg1);
    freeReg();
    freeReg();
    return 0;
}
...
//	std::cout << "Entering CodeGen as :- " << node->to_string() << "\n";
	switch (node->getNodeType()) {
		
		
		//
		//	THE ALL DREADED FUNCTION IS HERE
		//
		...
		case RETURN:{
			int reg1 = getReg();
			int reg2 = getReg();
			fprintf(output, "MOV R%d, -2 //Before Return Begin\n", reg1);
			fprintf(output, "MOV R%d, BP\n", reg2);
			fprintf(output, "ADD R%d, R%d\n",reg1,reg2);
			freeReg();
			...
			fprintf(output, "POP BP //Before Return End\n");			
			fprintf(output, "RET\n");
			freeReg();
			freeReg();
			break;
				...
				switch(getUsageType(entry)){
					case FUNCTION:{
						...
						for (int i=0;i<returnReg;i++)
							fprintf(output, "PUSH R%d //Before Function Call\n",i);
						int reg1 = getReg();
						int reg2 = getReg();
						if(returnReg)
							fprintf(output, "MOV R%d, SP\n",reg1);
						else
							fprintf(output, "MOV R%d, SP //Before Function Call\n",reg1);
						...
						fprintf(output, "CALL FUNC%s //Calling Function\n", node->getName().c_str());
						freeReg();
						
						reg1 = getReg();
						reg2 = getReg();
						fprintf(output, "MOV R%d, SP //After Function Return Begin\n", reg1);
						fprintf(output, "MOV R%d, %lu\n",reg2, entry->getArgs().size()+1);
						...
						if(returnReg)
							...
							fprintf(output, "MOV R%d, [R%d] //After Function Return End\n", returnReg, reg2);
						freeReg(debug);						
						freeReg(debug);						
						for (int i=returnReg-1;i>=0;i--)
							fprintf(output, "POP R%d //After Function Return End\n",i);
						return returnReg;
						break;
					}
					case VARIABLE:{
